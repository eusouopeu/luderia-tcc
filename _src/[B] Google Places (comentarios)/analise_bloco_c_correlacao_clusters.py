...
Lê _data/processed/bloco_c_avaliacoes_texto_filtrado.csv (saída de
build_bloco_c_texto_filtrado.py, que é quem calcula num_caracteres_texto).

Saídas:
    _data/processed/bloco_c_correlacao_spearman.csv
    _data/processed/bloco_c_correlacao_spearman_pvalor.csv
    _data/processed/bloco_c_correlacao_pearson.csv
    _data/processed/bloco_c_clusters.csv (dados originais + rótulo de cluster)
    _data/processed/bloco_c_clusters_perfil.csv (média das 4 variáveis por cluster)
...
IN_PATH = ROOT / "_data" / "processed" / "[B] Avaliacoes recentes" / "Google Places" / "bloco_c_avaliacoes_texto_filtrado.csv"
OUT_DIR = ROOT / "_data" / "processed" / "[B] Avaliacoes recentes" / "Google Places"
