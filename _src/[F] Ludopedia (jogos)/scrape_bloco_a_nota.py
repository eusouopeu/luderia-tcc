...
Ordem recomendada do fluxo completo:
    ...
    4. collect_bloco_a_details.py --ids-file _data/raw/ludopedia_top_ids.csv
...
Entrada: _data/processed/bloco_a_jogos.csv (coluna `link`, gerado por
build_bloco_a_dataset.py) — ou seja, roda só nos jogos que já passaram no
corte da amostra, não no catálogo inteiro.

Saída: _data/raw/ludopedia_nota.jsonl (resumível — ids já salvos são pulados)
...
IN_PATH = ROOT / "_data" / "processed" / "[F] Jogos (ludopedia)" / "bloco_a_jogos.csv"
OUT_PATH = ROOT / "_data" / "raw" / "[F] Jogos (ludopedia)" / "ludopedia_nota.jsonl"
...
def main():
    ...
    if errors:
        err_path = ROOT / "_logs" / "scrape_bloco_a_nota_errors.log"
