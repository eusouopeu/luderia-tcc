...
_data/raw/ludopedia_jogos.jsonl são pulados.

Por padrão lê _data/raw/ludopedia_ids.csv (catálogo inteiro, ~41 mil jogos).
Para limitar aos N jogos de maior nota (fluxo: scrape_ranking.py ->
select_top_n.py), passe --ids-file _data/raw/ludopedia_top_ids.csv.
...
IDS_PATH = ROOT / "_data" / "raw" / "[F] Jogos (ludopedia)" / "ludopedia_ids.csv"
OUT_PATH = ROOT / "_data" / "raw" / "[F] Jogos (ludopedia)" / "ludopedia_jogos.jsonl"
...
def main():
    ...
    if errors:
        err_path = ROOT / "_logs" / "collect_bloco_a_details_errors.log"
