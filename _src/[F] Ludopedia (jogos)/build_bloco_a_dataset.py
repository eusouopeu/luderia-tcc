...
Se _data/raw/ludopedia_top_ids.csv existir (gerado por select_top_n.py a
partir do ranking raspado), as colunas nota_media, nota_rank e
qt_avaliacoes de lá são incorporadas aqui.

Gera DOIS arquivos:
  - _data/processed/bloco_a_jogos.csv           -> só os que passam no corte
  - _data/processed/bloco_a_jogos_completo.csv  -> todos os coletados, sem
    aplicar nenhum critério de corte (mesmos indicadores calculados)
...
IN_PATH = ROOT / "_data" / "raw" / "[F] Jogos (ludopedia)" / "ludopedia_jogos.jsonl"
TOP_IDS_PATH = ROOT / "_data" / "raw" / "[F] Jogos (ludopedia)" / "ludopedia_top_ids.csv"
OUT_PATH = ROOT / "_data" / "processed" / "[F] Jogos (ludopedia)" / "bloco_a_jogos.csv"
OUT_PATH_COMPLETO = ROOT / "_data" / "processed" / "[F] Jogos (ludopedia)" / "bloco_a_jogos_completo.csv"
...
def main():
    ...
    OUT_PATH_COMPLETO.parent.mkdir(parents=True, exist_ok=True)
    df.to_csv(OUT_PATH_COMPLETO, index=False)
    print(f"OK: dataset completo (sem critérios de corte) salvo em {OUT_PATH_COMPLETO}")
    ...
    print(f"Após critérios de corte (edição nacional, qt_tem>={MIN_QT_TEM}, ano>={MIN_ANO}, com mecânica): {len(df_corte)} jogos")

    OUT_PATH.parent.mkdir(parents=True, exist_ok=True)
    df_corte.to_csv(OUT_PATH, index=False)
    print(f"OK: dataset processado (com corte) salvo em {OUT_PATH}")
