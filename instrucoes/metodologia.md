...
A ordem dos blocos segue a relevância para o plano. Os prefixos dos arquivos já gerados em `data/processed/` seguem o desenho anterior e foram mantidos.

| Bloco | Conteúdo | Prefixo em `data/processed/` |
|---|---|---|
| A | Estabelecimentos e cardápios | `bloco_b_` (arquivos existentes); `cardapios_` (novo) |
| B | Avaliações recentes | `bloco_c_` (arquivos existentes, a substituir pela nova coleta) |
| C | Histórico dos avaliadores | `avaliadores_` (novo) |
| D | Contagem direta de ocupação | `contagem_` (novo) |
| E | Bases públicas | `bases_` (novo) |
| F | Jogos | `bloco_a_` (arquivos existentes) |
...
| **3.2 Hierarquia de evidências** | Escala N1–N6 ([financeiro.md](financeiro.md) §2) e hierarquia de tipos de dados ([../CLAUDE.md](../CLAUDE.md) §2); justificativa da exclusão de questionários |
...
|---|---|---|
| IBGE — Censo 2022 (setor censitário) e estimativas populacionais | População e renda por setor censitário e município | Tamanho do público por capital e bairro; avaliações por 100 mil habitantes (5.2, 6.4, 6.6) |
...
| RAIS e Novo CAGED (MTE, portal PDET) | Remuneração média por ocupação (CBO) e município | Custo de pessoal (7.5 e cap. 9) |
| FipeZap — índice de locação comercial | Preço de locação por m² nas capitais cobertas | Aluguel (cap. 9) |
| Lei Complementar 123/2006 e anexos do Simples Nacional | Alíquotas por faixa de receita | Tributos (8.1 e cap. 9) |
| Banco Central — SGS e relatório Focus | Selic e IPCA observados e projetados | Taxa mínima de atratividade e inflação do modelo (cap. 9) |
...
- Scripts em `src/` seguindo o padrão `collect_*` → `build_*` → `analise_*`; saídas em `data/processed/` com o prefixo do bloco.
- Dados brutos (textos integrais e históricos) ficam em `data/raw/`, fora do repositório público. O TCC e o repositório divulgam **apenas dados processados**.
...
Custos (aluguel, pessoal, tributos) e indicadores seguem [financeiro.md](financeiro.md): registro de premissas com faixas, triangulação, tornado, ponto de ruptura e, opcionalmente, Monte Carlo.
